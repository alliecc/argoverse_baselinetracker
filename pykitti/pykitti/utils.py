    ...
    with open(filepath, 'r') as f:
        ...
        for line in f.readlines():
            ...
            if line_count < 4:
                ...
                key, value = line.split(' ', 1)
                try:
                    data[key] = np.concatenate ((np.array([float(x) for x in value.split()]).reshape(3,4), np.array([0,0,0,1])[np.newaxis,:]) ) 
...
def load_oxts_packets_and_poses(oxts_filename):
    ...
    oxts = []
    with open(oxts_filename, 'r') as f:
        for line in f.readlines():
            line = line.split()
